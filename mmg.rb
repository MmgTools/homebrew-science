class Mmg < Formula
  desc "Simplicial remeshers"
  homepage "https://www.mmgtools.org"
  url "https://github.com/MmgTools/mmg/archive/v5.2.5.tar.gz"
  sha256 "a39f82fe41dcc59fda92c58634397da527de6ca3ee3b99127b598a8ffebfd301"
  head "https://github.com/MmgTools/mmg.git"
  ...
  def install
    mkdir "build" do
      system "cmake", "-G", "Unix Makefiles", "..",
             "-DBUILD_TESTING=ON", "-DONLY_VERY_SHORT_TESTS=ON",
             *std_cmake_args

      if build.with?("scotch")
        system "cmake", "-DSCOTCH_DIR="+Formula["scotch"].opt_prefix,
               "-DUSE_SCOTCH=ON", ".."
        system "cmake", ".."
      else
        system "cmake", "-DUSE_SCOTCH=OFF", ".."
      end

      system "make"
      system "make", "install"
      system "ctest", "-VV" if build.with? "test"
    end
  end
  ...
  test do
    ...
    if build.with?("scotch")
      system ENV.cc, "test.c", "-L#{lib}", "-lmmg", "-lscotch", "-lscotcherr", "-o", "test"
    else
      system ENV.cc, "test.c", "-L#{lib}", "-lmmg", "-o", "test"
    end
    system "./test", "out.mesh"
  end
end
